...
async function presencaOnline(){
 ...
 const rows=(cr.data||[]).filter(c=>!c.is_test_account), pres=(pr.data||[]).filter(p=>!['comandante_financeiro','comandante','adm_financeiro'].includes((p.access_role||'').toLowerCase()));
...
function tipoAcessoBadge(role,test){ const r=(role||'').toLowerCase(); if(test) return '<span class="pill" style="border-color:#9b59b6;color:#9b59b6">👻 TESTE — MÉDIUM FANTASMA</span>'; if(r.includes('comandante')) return '<span class="pill" style="border-color:#d4a017;color:#d4a017">👑 COMANDANTE FINANCEIRO</span>'; return '<span class="pill">'+esc(role||'Médium')+'</span>'; }
...
async function visitasHoje(){
 ...
 if(vr.error){alert(vr.error.message);return} const visits=(vr.data||[]).filter(v=>!['comandante_financeiro','comandante','adm_financeiro'].includes((v.access_role||'').toLowerCase())); const children=(cr.data||[]).filter(c=>!c.is_test_account); const notices=nr.data||[], acks=ar.data||[];
...
async function criarAcessoFilho(){
 const r=await sb.from('children').select('id,full_name,house_role,profile_id,active,profiles:profile_id(id,username,role,active)').eq('active',true).order('full_name');
 if(r.error){alert('Não foi possível carregar os filhos: '+r.error.message);return}
 const rows=(r.data||[]).filter(x=>x.profile_id && x.profiles?.role==='filho');
 if(!rows.length){alert('Nenhum Filho com perfil cadastrado foi encontrado.');return}
 view().innerHTML='<div class="command-dashboard-head"><div><span class="eyebrow">ACESSO EXCLUSIVO DOS FILHOS</span><h2>🔐 Criar acesso individual</h2><p class="lead">Selecione o médium e gere o acesso exclusivo dele.</p></div><button class="btn secondary" onclick="membrosOperacao()">← Voltar</button></div>'+
 '<div class="command-card access-security-note"><h3>Como funciona</h3><p>Toque no <b>@</b>, escolha o médium e depois gere o código pessoal de 4 dígitos.</p></div>'+
 '<form id="childAccessForm" class="command-form"><label class="full">Filho / Médium<div style="display:flex;gap:8px"><input id="childAccessSearch" name="child_name" placeholder="Toque no @ para selecionar" readonly required><button id="childAccessAt" type="button" class="btn gold" aria-label="Abrir lista de médiuns">@</button></div><div id="childAccessPicker" class="notice-box" style="display:none;max-height:260px;overflow:auto"></div><input id="childAccessId" type="hidden" name="child_id"></label><label>Usuário de acesso<input id="childAccessUsername" name="username" readonly placeholder="Será gerado automaticamente"></label><label>Código de acesso<input name="pin" readonly placeholder="Será gerado pelo sistema"></label><div class="full"><button type="submit" class="btn gold">🔐 Gerar acesso exclusivo</button></div></form><div id="childAccessResult"></div>';
 const form=document.getElementById('childAccessForm'), search=document.getElementById('childAccessSearch'), user=document.getElementById('childAccessUsername'), id=document.getElementById('childAccessId'), picker=document.getElementById('childAccessPicker');
 picker.innerHTML='<input id="childAccessFilter" placeholder="🔎 Buscar médium..." style="width:100%;margin-bottom:8px">'+rows.map(x=>'<button type="button" class="btn outline child-access-option" data-id="'+esc(x.id)+'" style="display:block;width:100%;margin:5px 0;text-align:left">@ '+esc(x.full_name)+' <small>• '+esc(x.house_role||'Filho')+'</small></button>').join('');
 const choose=x=>{id.value=x.id;search.value=x.full_name;user.value=(x.profiles?.username&&x.profiles.username.endsWith('@'))?x.profiles.username:'';picker.style.display='none'};
 picker.querySelectorAll('.child-access-option').forEach(b=>b.onclick=()=>choose(rows.find(x=>x.id===b.dataset.id)));
 document.getElementById('childAccessAt').onclick=()=>{picker.style.display=picker.style.display==='none'?'block':'none';if(picker.style.display==='block')picker.querySelector('#childAccessFilter')?.focus()};
 document.getElementById('childAccessFilter').oninput=e=>{const q=e.target.value.toLowerCase();picker.querySelectorAll('.child-access-option').forEach(b=>b.style.display=b.textContent.toLowerCase().includes(q)?'block':'none')};
 form.onsubmit=async e=>{e.preventDefault();const x=rows.find(a=>a.id===id.value);if(!x){document.getElementById('childAccessResult').innerHTML='<div class="notice-box danger">Toque no @ e selecione um Filho.</div>';return}
 const btn=form.querySelector('button[type=submit]');btn.disabled=true;btn.textContent='⏳ Gerando acesso…';
 const z=await sb.functions.invoke('access-pin',{body:{action:'create_child_access',child_id:x.id}});
 btn.disabled=false;btn.textContent='🔐 Gerar acesso exclusivo';
 if(z.error||z.data?.error){document.getElementById('childAccessResult').innerHTML='<div class="notice-box danger"><b>Atenção:</b> '+esc(z.data?.error||'Não foi possível gerar o acesso.')+'</div>';return}
 user.value=z.data.username;form.elements.pin.value=z.data.access_pin;
 document.getElementById('childAccessResult').innerHTML='<div class="notice-box success"><h3>✅ Acesso criado</h3><p><b>Acesso:</b> '+esc(z.data.username)+'</p><p><b>Código acesso:</b> <span style="font-size:24px;letter-spacing:.18em">'+esc(z.data.access_pin)+'</span></p><p class="mini">Envie o código em privado ao Filho. O código não fica armazenado em texto aberto.</p></div>';
 await audit('create_child_access','profiles',x.profile_id,{child_id:x.id,username:z.data.username});};
}
window.criarAcessoFilho=criarAcessoFilho;
