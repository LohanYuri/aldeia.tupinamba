...
async function criarAcessoFilho(){
 ...
 const opts=rows.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.full_name)+' • '+esc(x.house_role||'Filho')+'</option>').join('');
 view().innerHTML='<div class="command-dashboard-head"><div><span class="eyebrow">ACESSO EXCLUSIVO DOS FILHOS</span><h2>🔐 Criar acesso individual</h2><p class="lead">O acesso pertence exclusivamente ao Filho selecionado. O código de 4 dígitos é a chave pessoal que dá acesso ao portal.</p></div><button class="btn secondary" onclick="membrosOperacao()">← Voltar</button></div>'+
 '<div class="command-card access-security-note"><h3>Como funciona</h3><p>1. Toque no campo <b>@</b> para abrir a lista de todos os médiuns/filhos cadastrados.</p><p>2. Selecione a pessoa.</p><p>3. O sistema cria ou substitui o acesso somente daquele Filho.</p><p>4. O código de 4 dígitos é mostrado uma única vez para o Comandante enviar em privado.</p></div>'+
 '<form id="childAccessForm" class="command-form"><label class="full">Filho / Médium<input id="childAccessSearch" list="childrenAccessList" name="child_id" placeholder="Toque em @ para escolher" required><button type="button" class="btn outline" style="margin-top:6px;width:max-content" onclick="document.getElementById(\'childrenAccessList\').showPicker?.()||document.getElementById(\'childAccessSearch\').focus()">@ Abrir lista de médiuns</button><datalist id="childrenAccessList">'+rows.map(x=>'<option value="'+esc(x.full_name)+'" data-id="'+esc(x.id)+'"></option>').join('')+'</datalist><small class="muted">A lista é exibida apenas dentro do painel protegido do Comandante.</small></label><label>Usuário de acesso<input id="childAccessUsername" name="username" readonly placeholder="Será gerado automaticamente"></label><label>Código de acesso<input name="pin" readonly placeholder="Será gerado pelo sistema"></label><div class="full"><button class="btn gold">🔐 Gerar acesso exclusivo</button></div></form><div id="childAccessResult"></div>';
 const form=document.getElementById('childAccessForm'), search=document.getElementById('childAccessSearch'), user=document.getElementById('childAccessUsername');
 function sync(){const q=String(search.value||'').trim();const x=rows.find(a=>a.full_name===q||a.id===q);if(x){search.value=x.full_name;user.value=(x.profiles?.username&&x.profiles.username.endsWith('@'))?x.profiles.username:''}}
 search.addEventListener('input',sync);
 form.onsubmit=async e=>{e.preventDefault();const q=String(search.value||'').trim();const x=rows.find(a=>a.full_name===q||a.id===q);if(!x){document.getElementById('childAccessResult').innerHTML='<div class="notice-box danger">Selecione um Filho pela lista @.</div>';return}
 ...
 document.getElementById('childAccessResult').innerHTML='<div class="notice-box success"><h3>✅ Acesso criado</h3><p><b>Acesso:</b> '+esc(z.data.username)+'</p><p><b>Código acesso:</b> <span style="font-size:24px;letter-spacing:.18em">'+esc(z.data.access_pin)+'</span></p><p class="mini"><b>Importante:</b> este código será mostrado agora para o Comandante. Envie-o em privado ao Filho. O sistema não armazena o código em texto aberto.</p></div>';
