...
type Body={action:"list"|"create"|"update"|"delete";id?:string;email?:string;password?:string;full_name?:string;username?:string;role?:"filho"|"adm"|"dirigente"|"financeiro";active?:boolean};
export default {fetch:withSupabase({auth:"user"},async(req,ctx)=>{
 ...
 if(body.action==="list"){
  ...
  return Response.json({users:(profiles||[]).map(p=>({...p,email:auth.users.find(u=>u.id===p.id)?.email||null}))});
 }
 if(body.action==="create"){
  ...
  await ctx.supabaseAdmin.from("audit_logs").insert({actor_id:callerId,action:"create_user",entity:"profiles",entity_id:created.user.id,details:{email:body.email,username:body.username,role:body.role,full_name:body.full_name}});
